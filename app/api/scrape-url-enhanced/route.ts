...
${sanitizedMarkdown}
  ...
  return {
    ...
    message: 'URL scraped successfully with Jina Reader fallback (no API key required)'
  };
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!FIRECRAWL_API_KEY) {
      console.log('[scrape-url-enhanced] FIRECRAWL_API_KEY missing, using Jina Reader fallback');
      const fallbackData = await scrapeWithJina(url);
